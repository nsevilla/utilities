...
def compute_2pt(data_filename,random_filename,config,binslop):
    ...
    np.savetxt('theta_counts_{}.csv'.format(binslop),th_counts)
    np.savetxt('wtheta_counts_{}.csv'.format(binslop),wth_counts)
...
def plot_wth(result,config,label,outfile_label,result_comp=None,label_comp=None):
    ...
    plt.legend()
    plt.savefig('thwth_{}.png'.format(outfile_label))
...
def main():
    ...
    parser.add_option("--data_filename",type="string",dest="data_filename",help="Data file name",default="/afs/ciemat.es/user/s/sevilla/projects/stellarlss_project/Y3MLLOZ_weighted.fits")
    ...
    parser.add_option("--min_sep",type="float",dest="min_sep",help="Minimum theta",default=2.0)
    parser.add_option("--max_sep",type="float",dest="max_sep",help="Maximum theta",default=220.0)
    ...
    result = compute_2pt(options.data_filename,options.random_filename,config,options.binslop)
    #compute_2pt_pix()
    plot_wth(result,config,label='Treecorr binslop {}'.format(options.binslop),outfile_label='treecorr_binslop_{}'.format{options.binslop})
